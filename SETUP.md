...
(`*.sops` files; recipients in repo-root `.sops.yaml`). All you need to transport
to a new machine is the **age private key** — then everything decrypts locally,
no `scp` from melnick. See [docs/ENGINEERING_DECISIONS.md](docs/ENGINEERING_DECISIONS.md)
(ED-0002) for the rationale.

```bash
# 1. Put the age private key in place (transport it out-of-band, NEVER commit it):
mkdir -p ~/.config/sops/age
#   scp it from a working machine, e.g.:
scp -O melnick:~/familyfund_db_backups/age-keys.txt ~/.config/sops/age/keys.txt
chmod 600 ~/.config/sops/age/keys.txt

# 2. Materialize every plaintext env from its encrypted twin (recreates the
#    .env -> .env.dev symlink automatically; writes files as 600):
...
`bin/secrets.sh` subcommands: `status`, `decrypt [name|all]`, `encrypt [name|all]`,
`edit <name>` (edit in `$EDITOR`, re-encrypts on save), `rekey` (re-wrap for new
`.sops.yaml` recipients), `recipient`. After changing a secret, run
`bin/secrets.sh encrypt <name>` and commit the updated `.sops` file.
