            ...
            <tr>
                ...
                <td>
                    <div class='btn-group'>
                        ...
                        <form action="{{ route('accountMatchingRules.resend-email', $accountMatchingRule->id) }}" method="POST" class="d-inline">
                            @csrf
                            <button type="submit" class="btn btn-ghost-primary" title="Resend Email" onclick="return confirm('Send email notification to {{ $accountMatchingRule->account->email_cc ?? "account" }}?')"><i class="fa fa-envelope"></i></button>
                        </form>
                        <form action="{{ route('accountMatchingRules.destroy', $accountMatchingRule->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
