@php
    ...
    // Mirrors the `fund.full` middleware guarding the rebalance/allocations routes
    // so we only render those icons to users who won't 403 on click.
    $hasFullAccess = in_array($fund->id, auth()->user()?->getAccessibleFundIds()['full'] ?? []);
@endphp
<form action="{{ route('funds.destroy', $fund->id) }}" method="POST" class="d-inline-block">
    ...
    <div class='btn-group'>
        ...
        @endcan
        @if($hasFullAccess && $primaryPortfolio)
